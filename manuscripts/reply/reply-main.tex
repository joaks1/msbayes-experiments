...
the true (correct) posterior supports models with many divergence-time
parameters given data generated under temporally random divergences, but we are
unable to accurately approximate the posterior.
In the latter case, numerical error is not the issue, because the true
...
If the cause is numerical approximation error, then the model is sound and we
need to increase our computational effort or improve our Monte Carlo
...
parameters.
The behavior of their approach also suggests that insufficient computation is
not the primary cause of the issue.

If support for models with few divergence events is caused by their larger
marginal likelihoods, we need to correct the model; improving the numerical
approximation machinery will only increase our ability to estimate the true
posterior that leads us to the wrong interpretation about evolutionary history.
