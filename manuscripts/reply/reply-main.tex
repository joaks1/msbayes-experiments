...
This affect of the priors on marginal likelihoods creates a penalty for
over-parameterization in Bayesian model choice.
However, for this inference problem, the uniform priors on divergence times can
create a strong preference for simple models that are often of interest to
biogeographers even if these models do not explain the data very well.
This phenomenon is extrinsic to the numerical approximation machinery of \msb;
with such prior assumptions, the true posterior will favor models with
few divergence events.
However, such results, even if correct from the perspective of Bayesian model
choice, do not provide the biogeographical insights that a researcher who
employs \msb seeks to gain.
For example, a biogeographer would like to interpret the posterior probability
of a simultaneous divergence model as an estimate of the probability that this
model is correct.
However, when such prior assumptions are made about divergence times, the
estimated posterior probability is better interpretted as the probability that
you cannot completely rule out that the simultaneous-divergence model can
explain the data.
...
divergences when using a prior on divergence times that is strongly
