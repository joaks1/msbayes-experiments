...
We also explore the predictions of the causes of the bias proposed by
...
By giving the data more weight relative to the prior, the goal of empirical
Bayesian parameter estimation is to estimate a distribution that will be peaked
near the true value(s) of the model's parameter(s).
...
what informs the posterior probability of that model from the data.
...
\change{
The strong preference for the narrowest priors on divergence times used by
\citet{Hickerson2013} suggest their approach is biased toward models with less
parameter space and, as a consequence, will estimate model-averaged posteriors
dominated by models that exclude true values of the model's parameters.
}
...
\change{
When using uniformly distributed priors, the alternative to priors that reflect
prior uncertainty is to risk excluding the true values one seeks to estimate,
as shown above.
Fortunately, continuous distributions more appropriate as priors for positive
...
erroneous support for highly clustered divergences when populations diverge
randomly over the last $8\globalpopsize$ generations.
...
detect temporal variation in divergences and reduces the bias toward models of
