...
reflect prior uncertainty, as shown above, is to risk excluding the truth
...
They found that under these prior conditions \msb has the power, assuming a
per-site rate of $1.92\e{-8}$ mutations per generation, to detect multiple
...
taxa.
Assuming a rate of mutation consistent with nuclear loci ($1\e{-9}$), with such
prior information the method can reliably reject one divergence event when
divergences are random over a period of 3 million generations.
However, as noted by \citet{Oaks2012}, inferring more than one divergence time
...
yet spurious).
Investigators should not limit their assessment of method's power to only
differentiating inferences of one event or more (i.e. $\numt{} = 1$ versus
$\numt{} > 1$).
...
It is important to determine the mechanism by which broad uniform priors cause
\msb to support clustered models of divergence.
...
smaller marginal likelihoods \citep{Jeffries1935,Lindley1957}.
...
Thus, it is not surprising in light of this fundamental statistical issue that
the method tends to support simple models.
...
However, there are some issues with their argument.
...
If we make all of these assumptions and assume the gene divergence times are
estimated without error, the Hickerson et al.'s \citeyear{Hickerson2013}
argument only applies to one of the three different priors used by
\citet{Oaks2012}.
...
divergence parameters with values younger than the estimated gene divergence
estimates.
...
While sampling error is always a reality, clearly it is not playing
a large role in the results of \citet{Oaks2012} or presented above.
...
Furthermore, the results above demonstrate the model-averaging
approach of \citet{Hickerson2013} is prefers models with narrower \divt{}
...
% This further suggests the primary cause is the uniform
...
could be explored, it clear that the marginal likelihoods under broad uniform
priors will be greater for simpler models.
