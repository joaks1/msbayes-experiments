...
{\sffamily
    ...
    Here we show that their methodology suffered from a fundamental error that
    renders some of their results invalid and the remainder difficult to
    interpret.
    Furthermore, we demonstrate how their approach still suffers from a bias
    toward models with lesser parameter space, which manifests in a dangerously
...
\newpage
\noindent Recently, this journal has served as a medium for an interesting
discussion of the potential pitfalls of approximate Bayesian methods of
comparative phylogeographical model choice.
...
\citet{Hickerson2013} has published a response to this paper where they present
...
We show that this argument is based on an empirical Bayesian perspective
towards model choice and questionable assumptions, and does not explain the
...
real-world conditions with highly informative priors.  \citet{Oaks2012} discuss
...
choice is much more questionable.
...
choice}
We were alarmed when we saw the results of the model-averaging approach
used by \citet{Hickerson2013}.
...
\citet{Hickerson2013}).
This concerned us because the narrowest \divt{} prior used by
...
parameter space, and as a consequence was biasing the method toward models that
...
does not reduce the method's bias toward models with lesser parameter space,
...
championed by \citet{Hickerson2013}.
...
space containing the truth, but still lacks power to detect random variation in
...
We agree with \citet{Oaks2012} that this is likely caused by the inappropriate
...
parameters, in addition to different priors over divergence models, could
