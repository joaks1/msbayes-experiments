...
co-distributed taxa provides an important tool for better understanding how
regional and global biogeographic processes affect biodiversity.
...
Thus, when the number of pairs is not trivial, the method samples over a large
number of divergence models of differing dimensionality (i.e., all the possible
partitions of \npairs{} pairs to 1--\npairs{} divergence-time parameters), and,
in so doing, estimates the posterior probability of each possible divergence
model.

For priors on divergence-time and demographic parameters, \msb uses continuous
uniform probability distributions
This causes  divergence models with more divergence-time parameters to
integrate over a \emph{much} greater parameter space with low likelihood yet
high prior density, resulting in small marginal likelihoods relative to models
with fewer divergence-time parameters.
Furthermore, \msb uses a discrete uniform prior over the number of divergence
events $1,2,\ldots,\npairs{}$.
Because there are many more possible divergence models for intermediate numbers
of divergence events, this imposes a prior on divergence models that strongly
disfavors such models and puts most of the prior mass on models with either
very few or very many divergence-time parameters (see Figure 5 of
\citet{Oaks2012}; for brevity we will refer to this prior as ``U-shaped'').
Given that models with many divergence-time parameters have small marginal
likelihoods due to the uniform priors on divergence-time and demographic
parameters, the U-shaped prior will strongly favor models with very
few divergence events.
As a result, even when highly informative uniform priors are used for
divergence-time and demographic parameters, \msb will often strongly support
biogeographically interesting models with a small number of divergence events
shared among the taxa when, in reality, divergences were random over broad
timescales (\citet{Oaks2012}).
...
taxa; a result congruent with intuition given the richness of the model
...
(\empModelUniform).
For each of these models, we simulated 2\e{7} samples from the prior, and
...
divergence.
For both analyses, we simulated 5\e{7} samples from the prior and retained an
...
While our modifications improve the behavior of the model, we urge caution when
...
We also urge caution when using \dppmsbayes due to the fact that ABC methods
