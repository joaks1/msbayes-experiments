...
The Dirichlet process has been used as a nonparametric Bayesian approach to
many inference problems in evolutionary biology
\citep{Lartillot2004,Huelsenbeck2007,HuelsenbeckSuchard2007,Ane2007,Heath2011,Heath2012}.
...
distribution (i.e., hyperprior) for \concentrationParam \citep{Escobar1995}.
...
\citet{Huang2011}) imposes an U-shaped prior over divergence
...
\numt{}, respectively \citep{Hickerson2006,Huang2011,Oaks2012}.
...
justified by the fact that the expected value of $\pi_b$ is
unrelated to sample size \popSampleSize{1}{} and \popSampleSize{2}{} and
thus exchangeable among pairs \citep{Hickerson2006,Huang2011}.
This is actually incorrect for three reasons.
First, the independence of the expected value of $\pi_b$ and sample size
breaks down as $\pi_b$ approaches its lower limit of zero (i.e., very recent or
no divergence between populations), which might be common given the timescales
of most phylogeographical studies.
Second, the entire vector of summary statistics \alignmentSS{}{} for each pair
...
(e.g., $SD(\pi-\theta_W)$).
Third, for variables to be exchangeable, they do not need to be independent,
...
model has a single divergence event \citep{Huang2011}, and (2) the reordering
increases the model's tendency to infer a single divergence event
\citep{Huang2011}, it seems possible that this approach could be biasing the
method \citep{Oaks2012}.
...
\citep[v1.1;][]{ABCtoolbox}, which \citet{Oaks2012} showed performs very
...
\begin{description}
    \item[\modelOld] a model to represent the original \msb implementation with
    ...
    \item[\modelUshaped] a model with the U-shaped prior of \msb on unordered
    ...
    \item[\modelUniform] a model with a uniform prior over unordered divergence
        models and exponential priors on divergence-time and demographic
        parameters; $\divTimeIndexVector \sim \priorUniform$, $\divTime{} \sim
        Exp(mean=2.887)$, $\ancestralTheta{} \sim Exp(mean=0.025)$,
        $\descendantTheta{1}{} \sim Exp(mean=0.025)$, and
        $\descendantTheta{2}{} \sim Exp(mean=0.025)$
    \item[\modelDPP] a model with a Dirichlet-process prior on ordered
...
empirical dataset of Philippine vertebrates analyzed by \citet{Oaks2012}, which
