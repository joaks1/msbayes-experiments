...
\change{
The goal would be to estimate the posterior probability of models ranging from
a single divergence-time parameter (i.e., simultaneous divergence of all
co-distributed taxa) to the fully generalized model in which each taxon
...
the Philippines caused by Pleistocene sea-level fluctuations caused
...
validation of Bayesian model choice when the full data are replaced by
insufficient summary statistics, and the recent demonstration that ABC
estimates of model posterior probabilities can be biased when insufficient
summary statistics are employed \cite{Rober2011}.}
...
particular interest to researchers who employ the method.
}
