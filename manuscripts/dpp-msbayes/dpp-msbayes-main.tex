...
\change{
As a result, the summary statistics being compared when calculating the Euclidean
distance between the observed data and each simulation are often from sequence
alignments of \emph{different} taxon pairs and/or loci.
More specifically, the summary statistics calculated from the observed sequence
alignments are being compared to summary statistics calulated from datasets
...
\change{
Thus, whenever the six parameters listed above vary across any of the taxa or
loci (i.e., nearly all empirical applications), the taxon-specific summary
statistics \alignmentSS{}{} are not exchangeable.
Hence, the reshuffling of the summary statistic vectors in not mathematically
valid.
The magnitude of the affect of this violaton of exchangeability is currently
unknown.
...
divergence event.
By definition, if the summary statistic vectors were exhangeable, the
reordering would not change the likelihood or posterior (barring sampling
error).
Thus, the results of Huang et al.\ \cite{Huang2011} suggest the
reordering of the statistics could be introducing a bias.
...
\change{
Also, in all of the simulation-based analyses described below, the
summary statistic vectors \emph{are} exchangeable, because all of the
simulated datasets have the same 
(1) numbers of sequences,
(2) length of sequences,
(3) numbers of loci,
(4) \hky model parameters (\hkyModel{}{}),
