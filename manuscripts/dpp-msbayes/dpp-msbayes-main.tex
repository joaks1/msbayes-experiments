...
MAP model when compared to the \empModelOld model.
...
detect random variation in divergence times, reducing the tendency to estimate
...
(Figure~\ref{figValidationModelChoicePsi} \&
\ref{figValidationModelChoiceOmega}).
Furthermore, better priors on parameters improves the method's power to detect
...
Furthermore, the coalescent and mutational models specified by these parameters
...
observed by \citet{Oaks2012}, we still some evidence of such sensitivity when
...
under the diffuse prior might be still be caused by small marginal likelihoods
...
We also urge caution when applying \dppmsbayes due to the fact that ABC methods
