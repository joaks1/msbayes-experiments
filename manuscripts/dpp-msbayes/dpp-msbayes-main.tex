...
We also applied the original \msBayes model (\empModelOld) with priors
...
the exponential prior of the \empModelDPP, and the mean prior on population
...
other (\npModelDPP) re-sorted the summary statistics of pair, losing the
identity of the taxa and thus sampled over unordered models of divergence.
...
(Figures~\ref{figValidationModelChoicePsi} \&
\ref{figValidationModelChoiceOmega}).
In contrast, the original is biased toward overestimating the posterior
...
(Figure~\ref{figPowerAccuracyExp}).
...
all the data models of \powerSeriesExp (Figure \ref{figPowerAccuracyExp}).
...
(Figure~\ref{figPowerAccuracyUniform}).
...
\ref{figPowerAccuracyUniform}).
The results are very similar when the four models are applied to the data
simulated under the \powerSeriesOld seires of models
(Figure~\ref{figPowerAccuracyOld}).
