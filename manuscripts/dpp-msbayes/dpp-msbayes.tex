...
\highLight{Need to discuss units of time and how they are scaled from units
    proportional to time to units proportional to mutations.}
...
at unique times.
Regardless of the number of divergence time parameters (\divTimeNum), each
\divTime{} within \divTimeVector is \iid as $\divTime{} \sim
...
\subsection*{The ABC implementation of the model}
...
(1) drawing values of all the model's parameters \hpvector{} from their respective
prior distributions,
(2) rescaling the divergence times in \divTimeMapVector from units
proportional to time to units proportional to the expected number
of mutations via Equation X,
(3) simulate a gene tree by drawing coalescent times from
a multi-population Kingman-coalescent model,
(4) simulating sequences along the gene tree under the
\hky substitution parameters of \hkyModelVector,
and (5) calculating population genetic summary statistics from the sequences
\ssVector{} (these are the same statistics calculated from the observed
