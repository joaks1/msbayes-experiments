...
\begin{frame}
    ...
    \begin{description}
        ...
        \item[\popSampleSize{i}{}] Number of genome copies sampled for each
                pair $i$; \popSampleSize{1}{i} sampled from population 1, and
                \popSampleSize{2}{i} sampled from population 2.
        \item[\nloci{i}] Number of DNA sequence loci collected from genome
            copies of pair $i$
        \item[\nlociTotal] Total number of unique loci
        \item[\alignment{i}{j}] Sequence alignment of locus $j$ for pair $i$
...
\begin{frame}[t]
    ...
    \]
    Each \locusMutationRateScalar{} is \iid as $\locusMutationRateScalar{} \sim
    Gamma(\locusRateHetShapeParameter, 1/\locusRateHetShapeParameter)$
    \[
        p(\locusMutationRateScalarVector \given \locusRateHetShapeParameter) =
    ...
    \only<3>{
        ...
        \begin{enumerate}
            ...
            \item multinomial distribution on the number of times each index of
                ...
                least once.
        \end{enumerate}
        Each \divTime{} within \divTimeVector is \iid as $\divTime{} \sim U(a, b)$
    }
    \only<4>{
        \bigskip
        Continuous uniform priors on everything else:
        \ancestralTheta{}, \descendantTheta{1}{},
...
\begin{frame}
    ...
    pairs
    \footnote{\shortfullcite{Hickerson2006}}\footnote{\shortfullcite{Huang2011}}
\end{frame}
